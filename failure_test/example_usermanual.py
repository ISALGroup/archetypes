...
MyUnit2 = Unit('MyUnit2')
MyUnit2.expected_flows_in = ['Debarked log']
MyUnit2.expected_flows_out = ['Test flow']
MyUnit2.coefficients = {'Power per t log' : 8}

def Debarkfunc_logs2(log_flow2, coefficients):
    log_amount = log_flow2.attributes['mass_flow_rate']
    electricity_amount = coefficients['Power per t log'] * log_amount/1000.
    wood_index = log_flow2.attributes['components'].index('Dry wood')
    moisture_index = log_flow2.attributes['components'].index('Water')
    wood_ratio = log_flow2.attributes['composition'][wood_index]
    moisture_ratio = log_flow2.attributes['composition'][moisture_index]
    wood_flow_amount = log_amount

    return [{'name' : 'Test flow', 'components' : ['Dry wood', 'Water'], 'composition' : [1-moisture_ratio, moisture_ratio] , 'mass_flow_rate' : wood_flow_amount,
                     'flow_type': 'Process stream', 'temperature' : 25, 'pressure':1 , 'In or out' : 'Out', 'Set calc' : False, 'Set shear' : False}]

MyUnit2.calculations = {'Debarked log' : Debarkfunc_logs2}


processunits.append(MyUnit)
processunits.append(MyUnit2)
