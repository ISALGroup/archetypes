...
This is the python model for market pulp production from recycled paper (the
"finishing" archetype, NAICS 322110 - Pulp mills, Recycling/finishing
...
Unit6 = Unit('Dewatering')
Unit6.expected_flows_in = ['Thick stock', 'Electricity (Dewatering)']
Unit6.expected_flows_out = ['Dewatered pulp (Dewatering)', 'Wastewater (Dewatering)']

Unit6.coefficients = {'Electricity per ton of paper' : 1.2, 'Dewatering moisture change' : 0.10, 'Dewatering moisture goal' : 0.78}

def dewatering(stock_flow, coeff):
    stock_amount = stock_flow.attributes['mass_flow_rate']
    pulp_amount = stock_amount*(stock_flow.attributes['composition'][stock_flow.attributes['components'].index('Dry pulp')] + stock_flow.attributes['composition'][stock_flow.attributes['components'].index('Additives')])
    water_in = stock_amount - pulp_amount

    elec_in = pulp_amount*coeff['Electricity per ton of paper']/1000. #kWh

    total_moisture = water_in / (water_in + pulp_amount)
    ...
    pulp_water_out = pulp_amount * (moisture_out/(1 - moisture_out))
    wastewater_out = water_in - pulp_water_out

    total_outlet_amount = pulp_amount + pulp_water_out

    heat_in = stock_flow.attributes['heat_flow_rate']
    cp_water = 4.2
    overall_temp = amb_t + (heat_in/(stock_amount*cp_water))
    waste_water_heat = (overall_temp - amb_t) * wastewater_out * cp_water
    dewatered_pulp_heat = heat_in - waste_water_heat
...
Unit6.calculations = {'Thick stock' : dewatering}

#Unit 7: Pulp Drying

Unit7= Unit('Dryer (pulp)')
Unit7.expected_flows_in = ['Dewatered pulp (Dewatering)', 'Electricity (Dryer, pulp)', 'Air (Dryer, pulp)', 'Steam (Dryer, pulp)']
Unit7.expected_flows_out = ['Dried pulp', 'Exhaust gas (Dryer, pulp)', 'Condensate (Dryer, pulp)']


Unit7.coefficients = {'Dry pulp moisture' : 0.1 , 'Air temperature' : 110, 'd_T' : 5, 'Exhaust gas temperature' : 100, 'Electricity (kWh) per ton of paper' : 21, 'Paper temperature' : 45, 'Loss' : 0.1}


def Dryerfunc_pulp(paper_pulp_flow, coeff):
...
Unit7.calculations = {'Dewatered pulp (Dewatering)' : Dryerfunc_pulp}
...
#unit_recap_to_file('pulp_finishing', allflows, processunits)

#utilities_recap('pulp_finishing', allflows, processunits)