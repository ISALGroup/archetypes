...
This is the python model for Kraft pulp mills. It can only be run with the 
...
Unit12.coefficients = {'Black liquor energy content (MJ/kg dry solids)' : 15, 'Carbon in solids' : 0.35, 'Outlet steam temperature' : 300, 'Efficiency' : 0.9}
...
def slakingfunc_greenliq(gl_flow, coeff):
    ...
    quicklime_amount = CaO_req  * wood_reference_flow/1000.
    quicklime_t = cao_t
    ql_cp = cao_kgcp
...
def limekiln_wlm(wlm_flow, coeff):
    ...
    caco3_amount = wlm_amount * wlm_flow.attributes['composition'][wlm_flow.attributes['components'].index('CaCO3')]
...
Unit18 = Unit('Dewatering (pulp)')
Unit18.expected_flows_in = ['Washed pulp (Washing 2)', 'Electricity (Dewatering)', 'Recycled pulp' ]
Unit18.expected_flows_out = ['Dewatered pulp', 'Wastewater (Dewatering)']
Unit18.required_calc_flows = 2
Unit18.coefficients = {'Electricity per ton of paper' : 274, 'Dewatering moisture change' : 0.10, 'Dewatering moisture goal' : 0.78} 
...
Unit18.calculations = (['Washed pulp (Washing 2)', 'Recycled pulp'], dewatering)


#Unit 19: Pulp Drying

Unit19= Unit('Dryer (pulp)')
Unit19.expected_flows_in = ['Dewatered pulp', 'Electricity (Dryer, pulp)', 'Air (Dryer, pulp)', 'Steam (Dryer, pulp)']
Unit19.expected_flows_out = ['Dried pulp', 'Exhaust gas (Dryer, pulp)', 'Condensate (Dryer, pulp)']

Unit19.coefficients = {'Dry pulp moisture' : 0.1 , 'Air temperature' : 110, 'd_T' : 5, 'Exhaust gas temperature' : 100, 'Electricity (kWh) per ton of paper' : 21, 'Paper temperature' : 45, 'Loss' : 0.1}
...
Unit19.calculations = {'Dewatered pulp (Dewatering)' : Dryerfunc_paper}


#Unit 20: Recycled pulper

Unit20= Unit('Pulper (recycled line)')
Unit20.expected_flows_in = ['Recycled paper', 'Electricity (Pulper, recycled)', 'Water (Pulper, recycled)', 'Steam (Pulper, recycled)']
Unit20.expected_flows_out = ['Pulped recycled', 'Condensate (Pulper, recycled)']

Unit20.coefficients = {'Consistency' : 0.15, 'Pulping temperature' : 40, 'Electricity per t paper' : 25}
...
Unit20.calculations = {'Recycled paper' : Pulprec_paper}

#Unit 21: Screening

Unit21 = Unit('Screening (recycled line)')
Unit21.expected_flows_in = ['Pulped recycled', 'Electricity (Screening, recycled)']
Unit21.expected_flows_out = ['Screened recycled pulp', 'Rejects (recycling)']
Unit21.coefficients = {'Rejects amount' : 0.02, 'Electricity per ton' : 23}
...
Unit21.calculations = {'Pulped recycled' : Screenrec_pulp}

#Unit 22: De inking

Unit22 = Unit('Deinking')
Unit22.expected_flows_in = ['Screened recycled pulp', 'Electricity (deinking)', 'Steam (deinking)', 'Water (deinking)', 'Additives (deinking)']
Unit22.expected_flows_out = ['Deinked pulp', 'Condensate (deinking)']
Unit22.coefficients = {'Additives quantity' : 0.05, 'Electricity per t' : 10, 'Deinking temperature' : 45, 'Consistency' : 0.015}
...
Unit22.calculations = {'Screened recycled pulp' : Deinking_pulp}


#Unit 23: Washing (recycled line)

Unit23= Unit('Washing (recycled line)')
Unit23.expected_flows_in = ['Deinked pulp', 'Electricity (washing, recycled)', 'Water (washing, recycled)']
Unit23.expected_flows_out = ['Recycled pulp', 'Wastewater (washing, recycled)']
Unit23.coefficients = {'Washwater ratio' : 1, 'Electricity per ton' : 10}
...
Unit23.calculations ={'Deinked pulp' : Washrec_pulp}

processunits = [Unit1,Unit2,Unit3,Unit4,Unit5,Unit6,Unit7,Unit8,Unit9,Unit10,
                Unit11,Unit12,Unit13,Unit14,Unit15,Unit16,Unit17,Unit18,Unit19,Unit20, Unit21,Unit22,Unit23]
