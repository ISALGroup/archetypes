...
This is the python model for sulfite paperboard mills. It can only be run with the 
...
Unit14 = Unit('Stock preparation')
Unit14.expected_flows_in = ['Washed pulp (Washing 2)' , 'Make-up water (Stock preparation)', 'Additives','Recycled pulp', 'Electricity (Stock preparation)' ]
Unit14.expected_flows_out = ['Thick stock']
Unit14.required_calc_flows = 2
Unit14.coefficients = {'Electricity per ton of paper' : 274, 'Stock consistency' : 0.04, 'Effluent per ton paper' : 10, 'Amount of additives per ton' : 50} 

def stock_prep(washed_flows, coeff):
    ...
    outlet_t = amb_t + ((wash_pulp_flow.attributes['heat_flow_rate'] + wash_rec_pulp_flow.attributes['heat_flow_rate'])/(total_outlet_amount*cp_stock))
    heat_in = (wash_pulp_flow.attributes['heat_flow_rate'] + wash_rec_pulp_flow.attributes['heat_flow_rate'])
...
Unit14.calculations = (['Washed pulp (Washing 2)', 'Recycled pulp'], stock_prep)

#Unit 15: Forming and press
Unit15 = Unit('Forming and press')
Unit15.expected_flows_in = ['Thick stock' , 'Electricity (Forming and press)']
Unit15.expected_flows_out = ['Formed paper', 'Wastewater (Forming and press)']
Unit15.coefficients = {'Electricity per ton of paper' : 238, 'Press consistency' : 0.4}
def form_and_press(thk_stock_flow, coeff):
    ...
    return [{'name' : 'Electricity (Forming and press)', 'components' : None, 'mass_flow_rate' : 0,
            ...
            {'name' : 'Formed paperboard', 'components' : ['Paper', 'Water'], 'composition': [paper_amount/total_paper_amount, (1 - paper_amount)/total_paper_amount], 'mass_flow_rate' : total_paper_amount,
...
Unit15.calculations = {'Thick stock': form_and_press}

#Unit 16: Drying

Unit16= Unit('Yankee Dryer (paperboard)')
Unit16.expected_flows_in = ['Formed paperboard', 'Electricity (Dryer, paperboard)', 'Air (Dryer, paperboard)', 'Steam (Dryer, paperboard)']
Unit16.expected_flows_out = ['Dried paperboard', 'Exhaust gas (Dryer, paperboard)', 'Condensate (Dryer, paperboard)']

Unit16.coefficients = {'Dry paperboard moisture' : 0.05 , 'Air temperature' : 80, 'd_T' : 5, 'Exhaust gas temperature' : 60, 'Electricity (kWh) per ton of paper' : 21, 'Paper temperature' : 45, 'Loss' : 0.1}
...
def Dryerfunc_paper(paper_pulp_flow, coeff):
    ...
    mu_m_out = coeff['Dry paperboard moisture']
    ...
    return [{'name' : 'Electricity (Dryer, paperboard)', 'components' : None, 'mass_flow_rate' : 0,
             'flow_type': 'Electricity', 'temperature' : 0,  'In or out' : 'In', 'elec_flow_rate' : electricity_amount ,  'Set calc' : False, 'Set shear' : False},     
            {'name' : 'Air (Dryer, paperboard)', 'components' : ['Air'], 'composition': [1], 'mass_flow_rate' : m_air,
                     'flow_type': 'Air', 'temperature' : amb_t, 'pressure':1 , 'heat_flow_rate' :0 ,'In or out' : 'In', 'Set calc' : False, 'Set shear' : False},
            {'name' : 'Condensate (Dryer, paperboard)', 'components' : ['Water'], 'composition': [1], 'mass_flow_rate' : m_steam,
            ...
            {'name' : 'Steam (Dryer, paperboard)', 'components' : ['Water'], 'composition': [1], 'mass_flow_rate' : m_steam,
                     'flow_type': 'Steam', 'temperature' : steam_t, 'pressure':2.7 , 'heat_flow_rate' :Q_steam_in ,'In or out' : 'In', 'Set calc' : False, 'Set shear' : False},
            {'name' : 'Exhaust gas (Dryer, paperboard)', 'components' : ['Water', 'Air'], 'composition': [exhaust_gas_water_ratio, 1 - exhaust_gas_water_ratio], 'mass_flow_rate' : exhaust_amount,
                     'flow_type': 'Exhaust', 'temperature' : exhaust_t, 'pressure':1 , 'heat_flow_rate' :Q_exhaust ,'In or out' : 'Out', 'Set calc' : False, 'Set shear' : False},
            {'name' : 'Dried paperboard', 'components' : ['Dry pulp', 'Water'], 'composition': [1 - mu_m_out, mu_m_out], 'mass_flow_rate' : paper_out_amount,
...
Unit16.calculations = {'Formed paperboard' : Dryerfunc_paper}


#Unit 17: Recycled pulper

Unit17= Unit('Pulper (recycled line)')
Unit17.expected_flows_in = ['Recycled paper', 'Electricity (Pulper, recycled)', 'Water (Pulper, recycled)', 'Steam (Pulper, recycled)']
Unit17.expected_flows_out = ['Pulped recycled', 'Condensate (Pulper, recycled)']

Unit17.coefficients = {'Consistency' : 0.15, 'Pulping temperature' : 40, 'Electricity per t paper' : 25}
...
Unit17.calculations = {'Recycled paper' : Pulprec_paper}

#Unit 18: Screening

Unit18 = Unit('Screening (recycled line)')
Unit18.expected_flows_in = ['Pulped recycled', 'Electricity (Screening, recycled)']
Unit18.expected_flows_out = ['Screened recycled pulp', 'Rejects (recycling)']
Unit18.coefficients = {'Rejects amount' : 0.02, 'Electricity per ton' : 23}
...
Unit18.calculations = {'Pulped recycled' : Screenrec_pulp}

#Unit 19: De inking

Unit19 = Unit('Deinking')
Unit19.expected_flows_in = ['Screened recycled pulp', 'Electricity (deinking)', 'Steam (deinking)', 'Water (deinking)', 'Additives (deinking)']
Unit19.expected_flows_out = ['Deinked pulp', 'Condensate (deinking)']
Unit19.coefficients = {'Additives quantity' : 0.05, 'Electricity per t' : 10, 'Deinking temperature' : 45, 'Consistency' : 0.015}
...
Unit19.calculations = {'Screened recycled pulp' : Deinking_pulp}


#Unit 20: Washing (recycled line)

Unit20= Unit('Washing (recycled line)')
Unit20.expected_flows_in = ['Deinked pulp', 'Electricity (washing, recycled)', 'Water (washing, recycled)']
Unit20.expected_flows_out = ['Recycled pulp', 'Wastewater (washing, recycled)']
Unit20.coefficients = {'Washwater ratio' : 1, 'Electricity per ton' : 10}
...
Unit20.calculations ={'Deinked pulp' : Washrec_pulp}
