...
This is the python model for Sulfite pulp mills. It can only be run with the 
...
amb_t = 20 
recovery_yield = 0.55
SO2_in_DL = 0.045 #g/kg WL
MgO_in_DL = 0.0145 #g/kg WL
green_liq_moist = 0.9
...
wood_reference_flow = 1000.
recycling_rate = 0.658
recycled_paper_flow = wood_reference_flow * normal_wood_in * recycling_rate
paper_moisture = 0.05
...
Unit4.coefficients = {'Rejects ratio' : rejects_a, 'Rejects heat content' : heat_bark}
...
Unit5 = Unit('Digester')
Unit5.expected_flows_in = ['Chips', 'Steam (digester)', 'Digestion liquor']
Unit5.expected_flows_out = ['Pulp mix']
Unit5.coefficients = {'Digestion liquor ratio' : 1, 'Digestion liquor temperature' : white_liquor_t, 'Heat loss' : 0.05,
                      'Steam temperature' : 170}


def Digesterfunc_chips(chips_flow, coeff):
    chips_amount = chips_flow.attributes['mass_flow_rate']
    white_liquor_amount = chips_amount*coeff['Digestion liquor ratio']
    white_liquor_temperature = coeff['Digestion liquor temperature' ]
    ...
    return [{'name' : 'Steam (digester)', 'components' : ['Steam'], 'composition': [1], 'mass_flow_rate' : steam_amount,
            ...
            {'name' : 'White liquor', 'components' : ['White liquor (dry)', 'Water'], 'composition': [0.0595, 0.9405], 'mass_flow_rate' : white_liquor_amount,
...
def Washingfiltersfunc_liqpulp(liq_pulp_flow, coeff):
    ...
    return [{'name' : 'Electricity (Washing filters)', 'components' : None, 'mass_flow_rate' : 0,
            ...
            {'name' : 'Brown liquor', 'components' : ['Chemicals', 'Digested wood', 'Water'], 'composition': [dry_white_liquor_amount/black_liquor_amount, digested_wood/black_liquor_amount, water_ratio_inbl], 'mass_flow_rate' : black_liquor_amount,
...
Unit8.coefficients = {'Rejects ratio' : 0.02, 'Eletricity per t of pulp' : 10, 'Rejects heating value' : heat_bark}
...
def Screening2func_washpulp(wash_pulp_flow, coeff):
    ...
    combustion_energy_rejects = coeff['Rejects heating value'] * rejects_amount
...
Unit9.coefficients = {'Bleaching cycles' : 3,  'Consistency' : 0.1, 'kg ClO2 per t pulp' : 15, 'kg O2 per t pulp' : 10, 'kg NaOH per t pulp' : 20, 'Bleaching rate' : 0.1, 'Electricity consumption per bleaching step and per t of pulp' : 3, 'Bleaching temperature' : 100, 'Losses' : 0.1}
...
Unit11.expected_flows_in = ['Brown liquor', 'Steam (MEE)']
Unit11.expected_flows_out = ['Brown liquor (strong)', 'Condensate (MLE, process)', 'Condensate (MLE, utility)']
Unit11.coefficients = {'Steam economy' : 2.5, 'Moisture out' : 0.25, 'Temperature steam in' : 120, 'Temperature liquor out' : 55, 'Latent heat of vaporization at given T' : 2202.1, 'Water Cp' : 4.2}
...
def Evaporatorfunc_blckliq(black_liquor_flow, coeff):
    ...
    return [{'name' : 'Steam (MEE)', 'components' : ['Water'], 'mass_flow_rate' : steam_in_amount,
            ...
            {'name' : 'Brown liquor (strong)', 'components' : ['Digested wood', 'Chemicals', 'Water'], 'composition': [dig_wood_out_ratio, chem_out_ratio , mu_out], 'mass_flow_rate' : strg_liq_out_amount,
...
Unit11.calculations = {'Brown liquor' : Evaporatorfunc_blckliq}

#Unit 12 : Recovery boiler definition  
Unit12 = Unit('Recovery boiler')
Unit12.expected_flows_in = ['Brown liquor (strong)', 'Air (Recovery boiler)', 'Water (Recovery boiler)']
Unit12.expected_flows_out = ['Smelt', 'Stack (Recovery boiler)', 'Steam (from Recovery boiler)']
Unit12.coefficients = {'Brown liquor energy content (MJ/kg dry solids)' : 15, 'Carbon in solids' : 0.35, 'Outlet steam temperature' : 300, 'Efficiency' : 0.9}
...
def Recoveryboilerfunc_strblkliq(str_blk_liq_flow, coeff):
    ...
    Q_combustion = blk_liq_dry_amount * coeff['Brown liquor energy content (MJ/kg dry solids)'] * 1000
#    print('Q combustion = ' + str(Q_combustion) + ' kJ')
    SO2_in = SO2_in_DL*chem_amount
    MgO_in = MgO_in_DL * chem_amount
    t_steam_out = coeff['Outlet steam temperature']
    t_smelt_out = 800
    t_stack = coeff['Outlet steam temperature']
    smelt_amount = SO2_in + MgO_in
    smelt_cp = 1.1
    ...
    return [{'name' : 'Water (Recovery boiler)', 'components' : ['Water'], 'mass_flow_rate' : m_steam,
            ...
            {'name' : 'Smelt', 'components' : ['SO2', 'MgO'], 'composition': [SO2_in/smelt_amount, MgO_in/smelt_amount], 'mass_flow_rate' : smelt_amount,
...
Unit12.calculations = {'Brown liquor (strong)' : Recoveryboilerfunc_strblkliq}

#Unit 13 : Smelt dissolver definition  
Unit13 = Unit('Smelt dissolver')
Unit13.expected_flows_in = ['Smelt', 'Water (Smelt dissolver)']
Unit13.expected_flows_out = ['Digestion liquor']
green_liq_moist = (1-(SO2_in_DL + MgO_in_DL))
...
def smeltdissolverfunc_smelt(smelt_flow, coeff):
    ...
    water_cp = 4.2
    smelt_cp = 1.1
    MgO_ratio = smelt_flow.attributes['composition'][smelt_flow.attributes['components'].index('MgO')]
    MgO_amount = MgO_ratio * smelt_amount
    SO2_ratio = smelt_flow.attributes['composition'][smelt_flow.attributes['components'].index('SO2')]
    SO2_amount = SO2_ratio * smelt_amount
    gl_amount = smelt_amount + water_in_amount
    MgO_in_gl = MgO_amount/gl_amount
    SO2_in_gl = SO2_amount/gl_amount
    total_Q = Q_smelt
    ...
    return [{'name' : 'Water (Smelt dissolver)', 'components' : ['Water'], 'mass_flow_rate' : water_in_amount,
             'flow_type': 'Water', 'temperature' : amb_t,  'In or out' : 'In', 'heat_flow_rate' : 0 ,  'Set calc' : False, 'Set shear' : False},     
            {'name' : 'Digestion liquor', 'components' : ['Water', 'SO2', 'MgO'], 'composition': [1 - (SO2_in_gl + MgO_in_gl), SO2_in_gl, MgO_in_gl], 'mass_flow_rate' :gl_amount,
...
#Unit 14: Stock preparation

Unit14 = Unit('Dewatering (pulp)')
Unit14.expected_flows_in = ['Washed pulp (Washing 2)', 'Electricity (Dewatering)', 'Recycled pulp' ]
Unit14.expected_flows_out = ['Dewatered pulp', 'Wastewater (Dewatering)']
Unit14.required_calc_flows = 2
Unit14.coefficients = {'Electricity per ton of paper' : 274, 'Dewatering moisture change' : 0.10, 'Dewatering moisture goal' : 0.78} 

def dewatering(washed_flows, coeff):
    wash_pulp_flow = washed_flows[0]
    washpulp_amount1 = wash_pulp_flow.attributes['mass_flow_rate']
    paper_amount1 = washpulp_amount1*wash_pulp_flow.attributes['composition'][wash_pulp_flow.attributes['components'].index('Dry pulp')]
    water_in1 = washpulp_amount1*wash_pulp_flow.attributes['composition'][wash_pulp_flow.attributes['components'].index('Water')]
    
    wash_rec_pulp_flow = washed_flows[1]
    washpulp_amount2 = wash_rec_pulp_flow.attributes['mass_flow_rate']
    paper_amount2 = washpulp_amount2*wash_rec_pulp_flow.attributes['composition'][wash_rec_pulp_flow.attributes['components'].index('Dry pulp')]
    water_in2 = washpulp_amount2*wash_rec_pulp_flow.attributes['composition'][wash_rec_pulp_flow.attributes['components'].index('Water')]
    
    paper_amount = paper_amount1 + paper_amount2
    water_in = water_in1 + water_in2
    
    elec_in = paper_amount*coeff['Electricity per ton of paper']/1000. #kWh

    total_moisture = water_in / (water_in + paper_amount)
    
    if total_moisture > (coeff['Dewatering moisture goal'] + coeff['Dewatering moisture change']):
        moisture_out = coeff['Dewatering moisture goal']
    else:
        moisture_out = total_moisture - coeff['Dewatering moisture change']
        
    pulp_water_out = paper_amount * (moisture_out/(1 - moisture_out))
    wastewater_out = water_in - pulp_water_out

    total_outlet_amount = paper_amount + pulp_water_out
    
    heat_in1 = wash_pulp_flow.attributes['heat_flow_rate']
    heat_in2 = wash_rec_pulp_flow.attributes['heat_flow_rate']
    total_heat = heat_in1 + heat_in2
    cp_water = 4.2
    overall_temp = amb_t + ( (heat_in1 + heat_in2)/cp_water)
    waste_water_heat = (overall_temp - amb_t) * wastewater_out * cp_water
    dewatered_pulp_heat = total_heat - waste_water_heat
    
    return[{'name' : 'Electricity (Dewatering)', 'components' : None, 'mass_flow_rate' : 0,
             'flow_type': 'Electricity', 'temperature' : 0,  'In or out' : 'In', 'elec_flow_rate' : elec_in ,  'Set calc' : False, 'Set shear' : False},     
            {'name' : 'Dewatered pulp (Dewatering)', 'components' : ['Water', 'Pulp'], 'composition': [moisture_out, 1 - moisture_out], 'mass_flow_rate' : total_outlet_amount,
                     'flow_type': 'Process flow', 'temperature' : overall_temp, 'pressure': 1 , 'heat_flow_rate' :dewatered_pulp_heat, 'combustion_energy_content' : 0 ,'In or out' : 'Out', 'Set calc' : True, 'Set shear' : False},
            {'name' : 'Wastewater (Dewatering)', 'components' : ['Water'], 'composition': [1], 'mass_flow_rate' : wastewater_out,
                     'flow_type': 'Process flow', 'temperature' : overall_temp, 'pressure': 1 , 'heat_flow_rate' :waste_water_heat, 'combustion_energy_content' : 0 ,'In or out' : 'Out', 'Set calc' : False, 'Set shear' : False}
        ]

Unit14.calculations = (['Washed pulp (Washing 2)', 'Recycled pulp'], dewatering)


#Unit 15: Pulp Drying

Unit15= Unit('Dryer (pulp)')
Unit15.expected_flows_in = ['Dewatered pulp', 'Electricity (Dryer, pulp)', 'Air (Dryer, pulp)', 'Steam (Dryer, pulp)']
Unit15.expected_flows_out = ['Dried pulp', 'Exhaust gas (Dryer, pulp)', 'Condensate (Dryer, pulp)']

Unit15.coefficients = {'Dry pulp moisture' : 0.1 , 'Air temperature' : 110, 'd_T' : 5, 'Exhaust gas temperature' : 100, 'Electricity (kWh) per ton of paper' : 21, 'Paper temperature' : 45, 'Loss' : 0.1}
...
def Dryerfunc_paper(paper_pulp_flow, coeff):
    ...
    dry_paper_amount = paper_pulp_amount * paper_pulp_flow.attributes['composition'][paper_pulp_flow.attributes['components'].index('Pulp')]
    water_amount = paper_pulp_amount - dry_paper_amount
    mu_m_out = coeff['Dry pulp moisture']
    ...
    return [{'name' : 'Electricity (Dryer, pulp)', 'components' : None, 'mass_flow_rate' : 0,
             'flow_type': 'Electricity', 'temperature' : 0,  'In or out' : 'In', 'elec_flow_rate' : electricity_amount ,  'Set calc' : False, 'Set shear' : False},     
            {'name' : 'Air (Dryer, pulp)', 'components' : ['Air'], 'composition': [1], 'mass_flow_rate' : m_air,
                     'flow_type': 'Air', 'temperature' : amb_t, 'pressure':1 , 'heat_flow_rate' :0 ,'In or out' : 'In', 'Set calc' : False, 'Set shear' : False},
            {'name' : 'Condensate (Dryer, pulp)', 'components' : ['Water'], 'composition': [1], 'mass_flow_rate' : m_steam,
            ...
            {'name' : 'Steam (Dryer, pulp)', 'components' : ['Water'], 'composition': [1], 'mass_flow_rate' : m_steam,
                     'flow_type': 'Steam', 'temperature' : steam_t, 'pressure':2.7 , 'heat_flow_rate' :Q_steam_in ,'In or out' : 'In', 'Set calc' : False, 'Set shear' : False},
            {'name' : 'Exhaust gas (Dryer, pulp)', 'components' : ['Water', 'Air'], 'composition': [exhaust_gas_water_ratio, 1 - exhaust_gas_water_ratio], 'mass_flow_rate' : exhaust_amount,
                     'flow_type': 'Exhaust', 'temperature' : exhaust_t, 'pressure':1 , 'heat_flow_rate' :Q_exhaust ,'In or out' : 'Out', 'Set calc' : False, 'Set shear' : False},
            {'name' : 'Dried pulp', 'components' : ['Dry pulp', 'Water'], 'composition': [1 - mu_m_out, mu_m_out], 'mass_flow_rate' : paper_out_amount,
...
Unit15.calculations = {'Dewatered pulp (Dewatering)' : Dryerfunc_paper}


#Unit 16: Recycled pulper

Unit16= Unit('Pulper (recycled line)')
Unit16.expected_flows_in = ['Recycled paper', 'Electricity (Pulper, recycled)', 'Water (Pulper, recycled)', 'Steam (Pulper, recycled)']
Unit16.expected_flows_out = ['Pulped recycled', 'Condensate (Pulper, recycled)']

Unit16.coefficients = {'Consistency' : 0.15, 'Pulping temperature' : 40, 'Electricity per t paper' : 25}
...
Unit16.calculations = {'Recycled paper' : Pulprec_paper}

#Unit 17: Screening

Unit17 = Unit('Screening (recycled line)')
Unit17.expected_flows_in = ['Pulped recycled', 'Electricity (Screening, recycled)']
Unit17.expected_flows_out = ['Screened recycled pulp', 'Rejects (recycling)']
Unit17.coefficients = {'Rejects amount' : 0.02, 'Electricity per ton' : 23}
...
def Screenrec_pulp(waste_pulp, coeff):
    ...
    water_in_paper_in = waste_pulp_amount - dry_pulp
    dry_paper_cp = 1.4
    water_cp = 4.2
...
Unit17.calculations = {'Pulped recycled' : Screenrec_pulp}

#Unit 18: De inking

Unit18 = Unit('Deinking')
Unit18.expected_flows_in = ['Screened recycled pulp', 'Electricity (deinking)', 'Steam (deinking)', 'Water (deinking)', 'Additives (deinking)']
Unit18.expected_flows_out = ['Deinked pulp', 'Condensate (deinking)']
Unit18.coefficients = {'Additives quantity' : 0.05, 'Electricity per t' : 10, 'Deinking temperature' : 45, 'Consistency' : 0.015}
...
Unit18.calculations = {'Screened recycled pulp' : Deinking_pulp}


#Unit 19: Washing (recycled line)

Unit19= Unit('Washing (recycled line)')
Unit19.expected_flows_in = ['Deinked pulp', 'Electricity (washing, recycled)', 'Water (washing, recycled)']
Unit19.expected_flows_out = ['Recycled pulp', 'Wastewater (washing, recycled)']
Unit19.coefficients = {'Washwater ratio' : 1, 'Electricity per ton' : 10}
...
Unit19.calculations ={'Deinked pulp' : Washrec_pulp}
...
FlowA = Flow('Logs',['Water', 'Bark', 'Wood'],'input', amb_t, 1, [wood_moisture, normal_bark_in, normal_wood_in], None , None, wood_reference_flow, np.nan, 0)
FlowA.set_calc_flow()
allflows.append(FlowA)

FlowB = Flow('Recycled paper',['Water', 'Paper'],'input', amb_t, 1, [paper_moisture, 1 - paper_moisture], None , None, recycled_paper_flow, np.nan, 0)
FlowB.set_calc_flow()
...
#unit_recap_to_file('recycled_paper_line', allflows, processunits)

#utilities_recap('recycled_paper_line', allflows, processunits)