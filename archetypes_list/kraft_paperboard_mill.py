...
This is the python model for Kraft paperboard mills. It can only be run with the 
...
Unit18.expected_flows_in = ['Washed pulp (Washing 2)' , 'Make-up water (Stock preparation)', 'Additives','Recycled pulp', 'Electricity (Stock preparation)' ]
Unit18.expected_flows_out = ['Thick stock']
Unit18.required_calc_flows = 2
Unit18.coefficients = {'Electricity per ton of paper' : 274, 'Stock consistency' : 0.04, 'Effluent per ton paper' : 10, 'Amount of additives per ton' : 50} 
...
Unit18.calculations = (['Washed pulp (Washing 2)', 'Recycled pulp'], stock_prep)
...
Unit20= Unit('Yankee Dryer (paperboard)')
Unit20.expected_flows_in = ['Formed paperboard', 'Electricity (Dryer, paperboard)', 'Air (Dryer, paperboard)', 'Steam (Dryer, paperboard)']
Unit20.expected_flows_out = ['Dried paperboard', 'Exhaust gas (Dryer, paperboard)', 'Condensate (Dryer, paperboard)']

Unit20.coefficients = {'Dry paperboard moisture' : 0.05 , 'Air temperature' : 80, 'd_T' : 5, 'Exhaust gas temperature' : 60, 'Electricity (kWh) per ton of paper' : 21, 'Paper temperature' : 45, 'Loss' : 0.1}
...
Unit20.calculations = {'Formed paperboard' : Dryerfunc_paper}


#Unit 21: Recycled pulper

Unit21= Unit('Pulper (recycled line)')
Unit21.expected_flows_in = ['Recycled paper', 'Electricity (Pulper, recycled)', 'Water (Pulper, recycled)', 'Steam (Pulper, recycled)']
Unit21.expected_flows_out = ['Pulped recycled', 'Condensate (Pulper, recycled)']

Unit21.coefficients = {'Consistency' : 0.15, 'Pulping temperature' : 40, 'Electricity per t paper' : 25}
...
Unit21.calculations = {'Recycled paper' : Pulprec_paper}

#Unit 22: Screening

Unit22 = Unit('Screening (recycled line)')
Unit22.expected_flows_in = ['Pulped recycled', 'Electricity (Screening, recycled)']
Unit22.expected_flows_out = ['Screened recycled pulp', 'Rejects (recycling)']
Unit22.coefficients = {'Rejects amount' : 0.02, 'Electricity per ton' : 23}
...
Unit22.calculations = {'Pulped recycled' : Screenrec_pulp}

#Unit 23: De inking

Unit23 = Unit('Deinking')
Unit23.expected_flows_in = ['Screened recycled pulp', 'Electricity (deinking)', 'Steam (deinking)', 'Water (deinking)', 'Additives (deinking)']
Unit23.expected_flows_out = ['Deinked pulp', 'Condensate (deinking)']
Unit23.coefficients = {'Additives quantity' : 0.05, 'Electricity per t' : 10, 'Deinking temperature' : 45, 'Consistency' : 0.015}
...
Unit23.calculations = {'Screened recycled pulp' : Deinking_pulp}


#Unit 24: Washing (recycled line)

Unit24= Unit('Washing (recycled line)')
Unit24.expected_flows_in = ['Deinked pulp', 'Electricity (washing, recycled)', 'Water (washing, recycled)']
Unit24.expected_flows_out = ['Recycled pulp', 'Wastewater (washing, recycled)']
Unit24.coefficients = {'Washwater ratio' : 1, 'Electricity per ton' : 10}
...
Unit24.calculations ={'Deinked pulp' : Washrec_pulp}

processunits = [Unit1,Unit2,Unit3,Unit4,Unit5,Unit6,Unit7,Unit8,Unit9,Unit10,
                Unit11,Unit12,Unit13,Unit14,Unit15,Unit16,Unit17,Unit18,Unit19,Unit20, Unit21,Unit22,Unit23,Unit24]
